...
describe('ProductOrder e2e test', () => {
  ...
  // const productOrderSample = {"quantity":11796,"totalPrice":11295.74};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/products',
      body: {"name":"utterly bond blind","description":"ha stand","price":849.91,"productSize":"XXL","image":"Li4vZmFrZS1kYXRhL2Jsb2IvaGlwc3Rlci5wbmc=","imageContentType":"unknown"},
    }).then(({ body }) => {
      product = body;
    });
    // create an instance at the required relationship entity:
    cy.authenticatedRequest({
      ...
      body: {"placedDate":"2024-06-03T23:57:41.630Z","status":"CANCELLED","totalPrice":12486.05,"paymentMethod":"CREDIT_CARD","paymentReference":"deter"},
    }).then(({ body }) => {
      shoppingCart = body;
    });
  });
  ...
  describe('new ProductOrder page', () => {
    ...
    it.skip('should create an instance of ProductOrder', () => {
      cy.get(`[data-cy="quantity"]`).type('13000');
      cy.get(`[data-cy="quantity"]`).should('have.value', '13000');

      cy.get(`[data-cy="totalPrice"]`).type('29095.77');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '29095.77');
