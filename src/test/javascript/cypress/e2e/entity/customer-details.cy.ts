...
describe('CustomerDetails e2e test', () => {
  ...
  // const customerDetailsSample = {"gender":"MALE","phone":"1-756-702-9566 x777","addressLine1":"likewise opulent","city":"East Verla","country":"Norway"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      method: 'POST',
      url: '/api/users',
      body: {"login":"R@b9H\\MkD2\\6EkBvjz","firstName":"Earlene","lastName":"McKenzie","email":"Mortimer45@hotmail.com","imageUrl":"near","langKey":"plus mecha"},
    }).then(({ body }) => {
      user = body;
    });
  });
  ...
  describe('new CustomerDetails page', () => {
    ...
    it.skip('should create an instance of CustomerDetails', () => {
      ...
      cy.get(`[data-cy="phone"]`).type('574.658.1416 x9714');
      cy.get(`[data-cy="phone"]`).should('have.value', '574.658.1416 x9714');

      cy.get(`[data-cy="addressLine1"]`).type('bend');
      cy.get(`[data-cy="addressLine1"]`).should('have.value', 'bend');

      cy.get(`[data-cy="addressLine2"]`).type('leash consequently');
      cy.get(`[data-cy="addressLine2"]`).should('have.value', 'leash consequently');

      cy.get(`[data-cy="city"]`).type('Douglastown');
      cy.get(`[data-cy="city"]`).should('have.value', 'Douglastown');

      cy.get(`[data-cy="country"]`).type('Costa Rica');
      cy.get(`[data-cy="country"]`).should('have.value', 'Costa Rica');
