...
describe('ShoppingCart e2e test', () => {
  ...
  // const shoppingCartSample = {"placedDate":"2024-06-03T18:38:46.005Z","status":"COMPLETED","totalPrice":19859.49,"paymentMethod":"IDEAL"};
  ...
  beforeEach(() => {
    ...
    cy.authenticatedRequest({
      ...
      body: {"gender":"MALE","phone":"(578) 618-4606 x641","addressLine1":"hmph","addressLine2":"sick poorly","city":"West Rhoda","country":"Samoa"},
    }).then(({ body }) => {
  ...
  describe('new ShoppingCart page', () => {
    ...
    it.skip('should create an instance of ShoppingCart', () => {
      cy.get(`[data-cy="placedDate"]`).type('2024-06-04T04:27');
      cy.get(`[data-cy="placedDate"]`).blur();
      cy.get(`[data-cy="placedDate"]`).should('have.value', '2024-06-04T04:27');

      cy.get(`[data-cy="status"]`).select('PAID');

      cy.get(`[data-cy="totalPrice"]`).type('25173.44');
      cy.get(`[data-cy="totalPrice"]`).should('have.value', '25173.44');

      cy.get(`[data-cy="paymentMethod"]`).select('IDEAL');

      cy.get(`[data-cy="paymentReference"]`).type('nibble ouch');
      cy.get(`[data-cy="paymentReference"]`).should('have.value', 'nibble ouch');
